...
**Schedules are in UTC.** All workflow cron expressions target AWST (UTC+8): 8am AWST = `0 0 * * 1-5` UTC.

**Critical N8N node rules:**
- HTTP nodes: use `specifyBody: "json"` + `contentType: "json"` — NEVER `"string"` + `"raw"` (sends empty Content-Type)
- HTTP nodes processing multiple items: add `"options": { "batching": { "batch": { "batchSize": 1 } } }` to prevent API overload
- Code nodes: use `this.helpers.httpRequest()` — `require('axios')` is disallowed
- Data flow: pass data forward through nodes (embed in request body) — `$('NodeName').first()` only returns first item
- Array delimiters: use `|` (pipe) between agent and workflow — comma breaks values like "Sydney, Australia"
- Always sync agency copy after editing: `cp base.workflow.json agency-base.workflow.json`

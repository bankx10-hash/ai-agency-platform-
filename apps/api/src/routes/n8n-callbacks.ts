...
7. Write like a human who genuinely cares that this person is losing — not a brand. Use "I" or "we", be direct, be urgent.${contentCalendar ? `\n\nCONTENT CALENDAR CONTEXT (draw topics and pillars from this):\n${contentCalendar.substring(0, 3000)}` : ''}`
    ...
    const message = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `${userMessage}.\n\nPlatforms: ${platformList.join(', ')}.\n\nReturn a single JSON object with keys for each platform. Each value must be an object with:\n- content: the full post text ready to copy-paste\n- hashtags: array of hashtag strings (with # prefix)\n- image_prompt: A prompt for AI image generation that pairs with this post.\n\nIMAGE PROMPT RULES (STRICT — follow every single one):\n1. ABSOLUTELY NO TEXT, WORDS, LETTERS, NUMBERS, OR WRITING of any kind in the image. Never include text overlays, captions, titles, quotes, watermarks, logos with text, or any written characters. This is the #1 rule.\n2. THE PERSON IS THE HERO: Every image must feature a person or people as the clear focal point. Show confident entrepreneurs, professionals in action, leaders making decisions. The viewer should see themselves in the image.\n3. Style: subtle cinematic photography — not Hollywood dramatic, but elevated and polished. Think high-end brand campaigns (Apple, Nike) where people look aspirational and relatable at the same time.\n4. Lighting: warm golden hour tones, soft cinematic color grading, gentle bokeh in the background. Slightly desaturated shadows with warm highlights. NOT flat corporate lighting, NOT harsh flash.\n5. Composition: shallow depth of field with the person sharp and background softly blurred. Shoot at eye level or slightly below to make the subject look powerful. Rule of thirds.\n6. Color palette: warm tones — amber, soft gold, deep navy, warm grays. Subtle teal-and-orange color grading is fine. Avoid sterile whites and cold blues.\n7. Expressions and poses: candid and confident, NOT stiff posed corporate headshots. Mid-action shots — typing, speaking, gesturing, thinking, walking with purpose.\n8. Settings: modern offices with warm lighting, coffee shops, city streets at golden hour, co-working spaces, rooftop meetings, standing at a window looking out.\n9. NEVER: text overlays, infographics, charts, diagrams, illustrations, cartoons, AI-looking renders, stock photo watermarks, collages, split screens, empty rooms without people.\n\nOnly include the platforms listed. Return valid JSON only, no markdown code blocks.`
      }]
    })
...
router.post('/:clientId/social/generate-images', async (req, res) => {
  ...
  try {
    ...
    for (const [platform, platformContent] of Object.entries(content)) {
      ...
      // Append style reinforcement — people-focused, subtle cinematic, no text
      const styleGuide = ', portrait of a confident professional person as the main subject, subtle cinematic photography, warm golden hour lighting, shallow depth of field, soft bokeh background, warm amber tones, teal and orange color grading, candid natural pose, shot on 85mm lens, NO TEXT, NO WORDS, NO LETTERS, NO WRITING, NO LOGOS, NO WATERMARKS, NO OVERLAYS, no illustrations, no cartoons, no infographics, no empty scenes without people'

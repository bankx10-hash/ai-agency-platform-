...
export class N8NService {
  ...
  private injectVariables(
    ...
    const replacements: Record<string, string> = {
      ...
      '{{APOLLO_API_KEY}}': esc(process.env.APOLLO_API_KEY || ''),
      '{{PERSON_TITLES}}': esc(config.personTitles || ''),
      '{{PERSON_LOCATIONS}}': esc(config.personLocations || ''),
      '{{KEYWORDS}}': esc(config.keywords || ''),
      '{{EMPLOYEE_RANGES}}': esc(config.employeeRanges || ''),

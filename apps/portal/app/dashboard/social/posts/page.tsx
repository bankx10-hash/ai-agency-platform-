    ...
    try {
      ...
      const res = await axios.post(
        `${API_URL}/social/posts/generate`,
        {
          ...
          contentPillar: formContentPillar || undefined,
          customImagePrompt: formImagePrompt || undefined,
        },
        { headers: { Authorization: `Bearer ${token}` } }
      )
                ...
                <div className="flex flex-col items-end gap-1 text-xs text-gray-500 dark:text-gray-400 flex-shrink-0">
                  ...
                  {(post.status === 'DRAFT' || post.status === 'SCHEDULED' || post.status === 'FAILED') && (
                    <button
                      ...
                      className="mt-2 inline-flex items-center gap-1 rounded px-2 py-1 text-xs text-red-500 hover:bg-red-50 hover:text-red-700 dark:hover:bg-red-900/20 dark:hover:text-red-400 transition-colors"
                      title="Delete post"
                    >
          ...
          <div className="relative z-10 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-6 shadow-xl mx-4">
            ...
            {(modalMode === 'create' || selectedPost?.status === 'DRAFT') && (
              <div className="mb-4 rounded-lg border border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-800/50">
                ...
                <div className="mb-3">
                  <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">
                    Custom Image Prompt (optional - describe exactly what image you want)
                  </label>
                  <textarea
                    value={formImagePrompt}
                    onChange={(e) => setFormImagePrompt(e.target.value)}
                    placeholder="e.g., A dentist smiling with a patient in a modern clinic, cinematic lighting, dark moody tones"
                    rows={2}
                    className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-y"
                  />
                </div>
                <div className="flex gap-2">
                  {modalMode === 'create' && (
